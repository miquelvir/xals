...
class RestaurantDefaultTableCollectionResource(MethodView):
    ...
    def get(self, restaurant_id) -> DefaultTableCollectionSchema.dict:
        ...
        default_tables: List[DefaultTable] = DefaultTable.query.filter_by(restaurant_id=restaurant_id).all()
        return DefaultTableCollectionSchema(
            default_tables=[default_table.to_schema() for default_table in default_tables]
        ).dict()
    ...
    def post(self, restaurant_id) -> NewDefaultTableOutSchema.dict:
        ...
        if DefaultTable.query.filter_by(name=name, restaurant_id=restaurant_id).one_or_none() is not None:
...
class RestaurantDefaultTableResource(MethodView):
    ...
    def delete(self, restaurant_id, name):
        ...
        default_table: DefaultTable = DefaultTable.query.filter_by(restaurant_id=restaurant_id, name=name).one_or_none()
